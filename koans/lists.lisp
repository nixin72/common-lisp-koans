...
(define-test car-and-cdr
  ...
  (let ((structure '((1 2) (("foo" . "bar")))))
    (assert-equal (list 1 2) (car structure))
    (assert-equal (list (cons "foo" "bar")) (car (cdr structure)))
    (assert-equal "bar" (cdr (car (car (cdr structure)))))
    ;; Lisp defines shorthand functions for up to four such nested calls.
    (assert-equal (list 1 2) (car structure))
    (assert-equal (list (cons "foo" "bar")) (cadr structure))
...
(define-test cons-tructing-improper-lists
  ...
  (let (;; We can construct non-cyclic improper lists using LIST*...
        ...
        (y '(6 7 8 9 . 0)))
    (assert-equal (cons 4 5) (last x))
    (assert-equal (cons 9 0) (last y)))
...
(define-test slicing-lists
  ...
  (let ((noms (list "peanut" "butter" "and" "jelly")))
    (assert-equal (list "peanut") (subseq noms 0 1))
    (assert-equal (list "peanut" "butter") (subseq noms 0 2))
    (assert-equal (list) (subseq noms 2 2))
    (assert-equal (list "and" "jelly") (subseq noms 2))))
