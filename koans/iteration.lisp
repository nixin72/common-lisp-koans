...
(define-test dotimes
    ...
    (let ((stack '()))
      (dotimes (i 5)
        (push i stack))
      (assert-equal (list 4 3 2 1 0) stack))
  ;; DOTIMES can optionally return a value.
  (let ((stack '()))
    (assert-equal (list 4 3 2 1 0) (dotimes (i 5 stack)
                                     (push i stack)))))
...
(define-test do
    ...
    (let ((result '()))
      (do ((i 0 (1+ i)))
          ((> i 5))
        (push i result))
      (assert-equal (list 5 4 3 2 1 0) result))
  ...
  (let ((result (do ((i 0 (1+ i))
                    ...
                    ((> i 5) (nreverse result))
                  (push i result))))
    (assert-equal (list 0 1 2 3 4 5) result)))
